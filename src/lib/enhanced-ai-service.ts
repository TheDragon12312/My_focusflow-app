...
class EnhancedAIService {
  ...
  private generatePersonalizedInsights(userStats: any): EnhancedAIInsight[] {
    ...
    const insightTemplates = [
      ...
      },
      {
        ...
        message: "De middag is een perfecte tijd om je focus te herwinnen. Een korte sessie kan wonderen doen!",
        action: "Start 25min sessie",
        priority: "medium" as const
      },
      // Gezondheidsadvies
      {
        condition: () => userStats.sessionsCompleted > 3 && userStats.focusTime > 90,
        type: "health" as const,
        title: "Tijd voor een pauze! 🌿",
        message: "Je hebt hard gewerkt vandaag. Een korte wandeling of stretch oefening kan je energie opnieuw opladen.",
      ...
      // Tijdspecifieke tips
      {
        ...
        message: "Dit is je gouden uur! De meeste mensen zijn 's ochtends het meest productief. Pak je moeilijkste taak aan!",
      ...
      },
      {
        ...
        message: "De middagdip is normaal. Probeer een korte wandeling of een gezonde snack voor meer energie.",
      ...
      // Algemene motivatie
      {
        ...
        message: "Kleine, consistente acties leiden tot grote resultaten. Je bent op de goede weg!",
      ...
      },
      {
        ...
        message: "Een reis van duizend mijl begint met een eerste stap. Start je eerste focus sessie vandaag!",
  ...
  async generateRealInsights(userStats: any, userId: string): Promise<EnhancedAIInsight[]> {
    ...
    try {
      ...
      console.log("✅ Personalized AI insights generated:", personalizedInsights);
      ...
      const fallbackInsight: EnhancedAIInsight = {
        ...
        message: "Je doet het geweldig! Elke focus sessie brengt je dichter bij je doelen.",
        actionable: true,
        action: "Ga verder",
        priority: "medium",
        timestamp: new Date(),
  ...
  async sendChatMessage(message: string, userId: string): Promise<string> {
    ...
    // Probeer eerst externe AI voor slimmere antwoorden
    ...
    try {
      const { data, error } = await supabase.functions.invoke('ai-coach-chat', {
        body: {
          message,
          chatHistory: this.chatHistory.slice(-10), // Laatste 10 berichten voor context
          userId
        }
      });
      ...
      console.log("External AI chat not available, using intelligent local responses");
      aiResponse = this.generateIntelligentResponse(message);
    }
  ...
  private generateIntelligentResponse(userMessage: string): string {
    ...
    const isQuestion = lowerMessage.includes('?') || lowerMessage.startsWith('hoe') || lowerMessage.startsWith('wat') || 
                     lowerMessage.startsWith('waarom') || lowerMessage.startsWith('wanneer') || lowerMessage.startsWith('waar');
    
    const isNegative = words.some(word => ['moe', 'uitgeput', 'demotiverend', 'moeilijk', 'lukt niet', 'probleem', 'stress', 'druk'].includes(word));
    const isPositive = words.some(word => ['goed', 'geweldig', 'super', 'top', 'succesvol', 'gelukt', 'blij', 'tevreden'].includes(word));
    
    // Specifieke onderwerpen detecteren
    if (lowerMessage.includes('focus') || lowerMessage.includes('concentratie') || lowerMessage.includes('aandacht')) {
      if (isQuestion) {
        if (lowerMessage.includes('verbeteren') || lowerMessage.includes('beter')) {
        ...
        } else if (lowerMessage.includes('lang') || lowerMessage.includes('tijd')) {
    ...
    if (lowerMessage.includes('motivatie') || lowerMessage.includes('gemotiveerd') || lowerMessage.includes('inspiratie')) {
      if (isNegative) {
    ...
    if (lowerMessage.includes('moe') || lowerMessage.includes('energie') || lowerMessage.includes('uitgeput')) {
      if (isQuestion) {
    ...
    if (lowerMessage.includes('planning') || lowerMessage.includes('organiseren') || lowerMessage.includes('structuur')) {
      if (isQuestion) {
    ...
    if (lowerMessage.includes('pauze') || lowerMessage.includes('rust') || lowerMessage.includes('ontspanning')) {
    ...
    if (lowerMessage.includes('wie ben je') || lowerMessage.includes('wat ben je') || lowerMessage.includes('help')) {
